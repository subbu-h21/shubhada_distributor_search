...
- **2026-07-14 (this session)**:
  ...
  - **Manual Pick UI** — On NOT_FOUND result rows, the frontend now displays each candidate SKU (from `debug.candidates`) as a clickable pill. Tapping a pill calls `POST /api/extract/manual-pick { history_id, target_id, candidate_name }`, which re-runs *just* that distributor with `force_candidate_name` set. The Sunshop/Chethana/Liveconnect adapters skip their scorer and select the matching suggestion by canonical name. The history entry is updated in DB; the ResultCard swaps to SUCCESS in-place.
