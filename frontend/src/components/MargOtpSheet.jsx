...
import { MargAPI } from '../lib/api';
import { Loader2, CheckCircle2, KeyRound, Trash2 } from 'lucide-react';
import { toast } from 'sonner';

/**
 * MARG (margcompusoft.com/eRetail) OTP session manager.
 * Step 1: mobile number → /marg/session/begin (server enters mobile, clicks
 *          "Login with OTP" so MARG texts an OTP to the phone).
 * Step 2: user types OTP → /marg/session/verify → cookies saved.
 */
const MargOtpSheet = ({ open, onOpenChange, onSaved }) => {
  ...
  useEffect(() => {
    if (open) {
      ...
      MargAPI.status().then(setStatus).catch(() => setStatus({ active: false }));
    }
  }, [open]);
  ...
  const sendOtp = async () => {
    ...
    setBusy(true);
    try {
      const r = await MargAPI.begin(mobile.trim());
  ...
  const verifyOtp = async () => {
    ...
    setBusy(true);
    try {
      await MargAPI.verify(pendingId, otp.trim());
      toast.success('MARG session saved');
      setPhase('done');
      const s = await MargAPI.status();
      setStatus(s);
      onSaved && onSaved();
    } catch (e) {
  ...
  const clearSession = async () => {
    if (!window.confirm('Log out of MARG? You will need to enter OTP again.')) return;
    setBusy(true);
    try {
      await MargAPI.clear();
      const s = await MargAPI.status();
      setStatus(s);
      toast('MARG session cleared');
      setPhase('mobile'); setOtp(''); setPendingId(null);
    } finally { setBusy(false); }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md border-emerald-600 rounded-sm p-0 gap-0" data-testid="marg-dialog">
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-neutral-200">
          ...
          <p className="mt-2 text-[11px] text-neutral-500 mono-track-wide font-medium">
            SHARED LOGIN FOR ALL MARG SUPPLIERS
          </p>
        </DialogHeader>
        ...
        <div className="p-6 space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button type="button" onClick={verifyOtp} disabled={busy}
                  ...
                  data-testid="marg-verify-otp-btn">
                  {busy && <Loader2 className="w-4 h-4 animate-spin" />}
                  VERIFY OTP
                </button>
              </div>
          ...
          {phase === 'done' && (
            <div className="text-center py-4">
              ...
              <div className="text-[11px] text-neutral-500 mt-1">Scraping across MARG suppliers is now enabled.</div>
          ...
          <p className="text-[10px] text-neutral-400 mono-track-wide leading-relaxed">
            Cookies typically stay valid for a day. If a scrape fails with SESSION_EXPIRED, re-authenticate here.
          </p>
        </div>
      </DialogContent>
    </Dialog>
